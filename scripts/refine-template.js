...
2. **Text Area Bounding Boxes & Positioning**:
   ...
   - Configure appropriate 'fontSize' (MUST be at least 30px or roughly 4% of the image height for legibility, larger if space permits), 'color' (MUST ALWAYS BE "white"), 'stroke' (MUST ALWAYS BE "black"), 'uppercase', 'textAlign', and 'fontFamily'.
...
CRITICAL: Return ONLY a raw valid JSON object for config.json matching this exact schema (no markdown, no backticks, no code blocks):
{
  ...
  try {
    ...
    // Enforce minimum font size of 30px or 4% of image height, whichever is larger, and white color + black stroke
